                ...
                char __user *buf, size_t len, loff_t *off)
{
  ...
  //reading GPIO value
  gpio_state = gpiod_get_value(my_gpio);
...
static int __init etx_driver_init(void)
{
  ...
  my_gpio = gpiod_get(NULL, "GPIO_23", GPIOD_OUT_LOW);
  //Requesting the GPIO
  if(IS_ERR(my_gpio)){
    pr_err("ERROR: GPIO 23 request\n");
    goto r_gpio;
  }
  ...
  */
  if(gpiod_export(my_gpio, false) != 0)
	{
		pr_err("Cannot export GPIO to sysfs\n");
		goto r_gpio;
	}
